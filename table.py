        ...
        for index, table in enumerate(tables):
            ...
            for mot_cle in mots_cles:
                if mot_cle in table_text:
                    ...
                    # Construct the Excel file name for this table
                    excel_filename = os.path.join(keyword_directory, f"{com_number}_{title_text}.xlsx")
                    wb.save(excel_filename)

        print(f"All tables from URL '{url}' have been saved in their respective keyword folders with cell borders.")
    else:
        print(f"La requête pour URL '{url}' a échoué avec le code d'état {response.status_code}")
